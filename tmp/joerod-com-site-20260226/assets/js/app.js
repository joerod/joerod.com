...
var custom_raw_videos = null;
var promo_videos = [
  { id: 'sOQKRjP3ZJk' },
  { id: '6OG3ZQKgHbI' }
];
var promo_window_start = new Date(2026, 6, 7, 0, 0, 0, 0);
var promo_window_end = new Date(2026, 8, 8, 0, 0, 0, 0);
var xmas_videos = [
  { id: 'rgEP1niScLc' },
  { id: 'S7OWoc-j8qQ' },
  { id: 'hZ9q3PtiYu8' },
...
function pickVideoList() {
  const now = new Date();
  const promoActive = now >= promo_window_start && now < promo_window_end;
  if (promoActive) {
    return promo_videos;
  }

  if (custom_raw_videos && Array.isArray(custom_raw_videos) && custom_raw_videos.length) {
    const promoIds = new Set(promo_videos.map((v) => v.id));
    const rawIds = new Set(custom_raw_videos.map((v) => v && v.id).filter(Boolean));
    const matchesPromoSet =
      custom_raw_videos.length === promo_videos.length &&
      promo_videos.every((v) => rawIds.has(v.id)) &&
      rawIds.size === promoIds.size;
    if (matchesPromoSet) {
...
(function boot() {
  ...
  (async () => {
    try {
      ...
      const rawVideos = data && data.youtube && data.youtube.rawVideos;
      if (Array.isArray(rawVideos)) {
        custom_raw_videos = rawVideos;
      }
